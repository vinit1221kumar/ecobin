...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const user = await adminLogin(email, password, secretKey);
        ...
        <div className="rounded-lg p-10" style={{ backgroundColor: '#1E293B', border: '2px solid #334155' }}>
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl md:text-4xl mb-2" style={{ color: '#FFD54F' }}>
              <BlurText text="Admin Login" style={{ color: '#FFD54F' }} blurAmount={3} />
            </h2>
            <p className="text-sm md:text-base font-semibold" style={{ color: '#94A3B8' }}>Secure admin access portal</p>
          </div>
          ...
          <div className="text-center mt-6" style={{ color: '#94A3B8' }}>
            <p>
              Don't have an admin account? <Link to="/admin/register" className="font-semibold hover:underline" style={{ color: '#00B8A9' }}>Register as Admin</Link>
            </p>
            <p className="mt-2">
